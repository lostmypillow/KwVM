...
CREATE TABLE vm_details (
    ...
    CONSTRAINT chk_optional_fields CHECK (
        pve = 0
        OR (
            pve_token_username IS NOT NULL
            AND pve_token_name IS NOT NULL
            AND pve_token_value IS NOT NULL
            AND pve_vm_id IS NOT NULL
        )
    ),
    CONSTRAINT uq_pve_vm_id UNIQUE (pve_vm_id),  -- Ensure pve_vm_id is unique
    CONSTRAINT uq_spice_proxy UNIQUE (spice_proxy),  -- Ensure spice_proxy is unique
...
VALUES (
    ...
    'lostmypillow', 
    'lostmypillow', 
    '245a4a7a-581f-434d-be48-e393d9578aa0',  -- Example UUID
    301, 
    'pve1.kaowei.tw:3128', 
    '192.168.2.13:3128',
    NULL,
